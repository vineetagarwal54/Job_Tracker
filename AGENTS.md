...
src/
  ...
  constants.js      # enums, config objects, getEmptyForm()/getEmptyProfile() factories, sample data
...
extension/           # separate browser extension for autofill on job sites (unrelated to resume gen)
resume/
...
(`{ workspaces, activeWorkspaceId, jobs, applicationProfiles }`) in a single
...
per-field on edit. See `ApplicationProfileForm.jsx` / `JobForm.jsx`.
...
- Template fits one page with **~5.15 lines of free space** (61.63pt) below
  the last baseline (page height 792pt, last content baseline at 730.37pt) —
  down from the old "~6 lines" estimate
