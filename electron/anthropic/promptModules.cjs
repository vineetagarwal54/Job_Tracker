...
const RESUME_TAILOR = `The selected canonical resume is authoritative. Propose only a minimal diff against it, never a complete resume. Prefer no change unless an approved candidate has a concrete, positive JD benefit above the supplied threshold. Prioritize explicit must-haves, repeated technical terms, and stronger verified evidence. Do not make cosmetic changes, stuff one-off keywords, or trade quantified evidence for weaker keyword-only wording. Use only supplied candidate, canonical, and content-bank IDs. You may propose at most three bullet changes, one one-for-one project swap, and four verified skill additions or swaps. Never remove experience, education, sections, skills, bullets without replacement, or projects without replacement. Never output LaTeX, headings, contact information, or document structure.`;
