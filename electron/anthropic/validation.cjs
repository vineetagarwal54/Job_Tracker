function sanitizeJob(input) {
  ...
  for (const key of Object.keys(input)) if (!["title", "company", "description", "resumeOption", "baseResumeId"].includes(key)) throw new Error(`Unknown job field '${key}'.`);
  ...
  if (result.description.length > 100000) throw new Error("Job description is too long.");
  result.resumeOption = typeof input.resumeOption === "string" ? input.resumeOption.trim() : "";
  result.baseResumeId = typeof input.baseResumeId === "string" ? input.baseResumeId.trim() : "";
  return result;
}
