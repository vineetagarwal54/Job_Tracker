...
function proposalFor(body, testCase) {
  ...
  const requirements = (testCase.must.length ? testCase.must : ["reliable software delivery"]).map((text, index) => {
    ...
    return { id: `req-${index + 1}`, text, priority: "must", kind: "technical-skill", evidenceExpectation: "knowledge", currentEvidenceIds: unsupported ? [] : [currentIds[index % currentIds.length]], candidateEvidenceIds: [], knowledgeSkillIds: [] };
  });
  return { version: 2, baseResumeId, roleFamily: testCase.name, seniority: "entry", blockers: [], requirements };
}
