...
async function generateResumeSelection({ client, apiKey, bank, base, job, analysis, extraction, coverage, relevancePlan, signal, generateDir, progress }) {
  ...
  const response = await client.request({ apiKey, signal, stream: true, timeoutMs: 240000, body: { model: MODEL, max_tokens: 5000, system: [{ type: "text", text: stable, cache_control: { type: "ephemeral" } }], output_config: { format: { type: "json_schema", schema: tailoringDiffSchema(bank, base, relevancePlan) } }, messages: [{ role: "user", content: dynamic }] } });
  ...
  const applied = applyTailoringDiff({ bank, base, diff: proposedDiff, extraction, analysis, relevancePlan });
