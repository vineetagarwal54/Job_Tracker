...
async function generateResumeSelection({ client, apiKey, bank, base, job, analysis, extraction, coverage, relevancePlan, signal, generateDir, progress }) {
  ...
  const dynamic = JSON.stringify({ selectedBase: base, job: { title: job.title, description: job.description }, analysis, baseCoverage: compactCoverage(coverage), meaningfulGaps, unsupportedMissing: promptPlan.unsupportedMissing, approvedCandidates: approvedCandidateContext(bank, base, promptPlan), minimumBenefit: promptPlan.minimumBenefit, instruction: "Prefer no changes. Put ordinary selections in changes. Put only bullet-rewrite selections in bulletRewrites with rewrittenText. Select only approved candidateId values. Select at most 3 bullet candidates total including rewrites, at most 1 project candidate, and at most 4 skill candidates. Keep every justification to one short sentence naming the concrete JD reason. All referenced IDs and evidence are resolved deterministically after selection." });
  let response;
  const apiStartedAt = Date.now();
  try {
    response = await client.request({ apiKey, signal, stream: true, timeoutMs: 120000, body: { model: MODEL, max_tokens: 4000, thinking: { type: "disabled" }, system: SELECTION_SYSTEM, output_config: { effort: "low", format: { type: "json_schema", schema: tailoringDiffSchema(bank, base, relevancePlan) } }, messages: [{ role: "user", content: dynamic }] } });
  } catch (error) {
