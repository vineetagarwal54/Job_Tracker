...
function registerResumeIpc() {
  ...
  ipcMain.handle("resume:status", async () => ({ api: keyProvider.status(), tectonic: await checkTectonic(), profile: (() => { const profile = getDefaultProfile(); const missing = []; if (!profile) return { ready: false, missing: ["default Application Profile", "name", "email"] }; if (![profile.fullName, profile.firstName, profile.lastName].some((value) => String(value || "").trim())) missing.push("name"); if (!String(profile.email || "").trim()) missing.push("email"); return { ready: missing.length === 0, missing }; })(), outputDisplayPath: paths.displayPath, models: MODELS }));
