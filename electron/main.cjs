...
function serializeResumeError(error) {
  ...
  return { code, message: code === "VALIDATION_FAILED" ? validationMessage : code === "MALFORMED_RESPONSE" ? malformedMessage : safeMessages[code] || "Resume generation failed.", status: error?.status || null, retryable: Boolean(error?.retryable), stage: error?.stage || null, detail: detail || null };
}
