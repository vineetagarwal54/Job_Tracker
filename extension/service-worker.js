...
// These files are injected into the target page frames in this exact order.
// They attach helpers to globalThis.JobTrackAutofill inside Chrome's isolated
...
// Open the side panel when the user clicks the action button.
// We do not inject anything on page load. Scanning and filling happen only
// in response to explicit messages from the side panel.
chrome.runtime.onInstalled.addListener(() => {
  chrome.sidePanel
    .setPanelBehavior({ openPanelOnActionClick: true })
    .catch((err) =>
      console.error("[JobTrack] sidePanel.setPanelBehavior failed:", err)
    );
});

// Returns a friendly reason if the URL is one Chrome blocks extensions on,
// or null if the page is scannable.
function pageBlockReason(url) {
  if (!url) return "The active tab has no URL yet.";
  if (url.startsWith("chrome://")) return "Chrome blocks extensions on chrome:// pages.";
  if (url.startsWith("chrome-extension://")) {
    return "Chrome blocks extensions on extension pages.";
  }
  if (url.startsWith("view-source:")) {
    return "Chrome blocks extensions on view-source: pages.";
  }
  if (url.startsWith("about:")) return "Chrome blocks extensions on about: pages.";

  if (
    /^https?:\/\/(chrome\.google\.com\/webstore|chromewebstore\.google\.com)\b/i.test(
      url
    )
  ) {
    return "Chrome blocks extensions on the Web Store.";
...
// Picks the user's actual web tab. lastFocusedWindow excludes the side panel
// and devtools windows so we do not accidentally scan our own UI.
async function getActiveWebTab() {
...
async function scanActiveTab() {
  ...
  if (!tab?.id) {
    return {
      error: "No active tab to scan.",
      hint: "Open a job application page and try again.",
    };
  }
  ...
  try {
    // allFrames so ATS forms in iframes are caught.
    const injection = await chrome.scripting.executeScript({
      target: { tabId: tab.id, allFrames: true },
      func: detectFields,
    });
    ...
    func: async (incomingPlans) => {
      ...
      if (!JT?.fillFields) {
        return incomingPlans.map((plan) => ({
          ...
          success: false,
          skipped: false,
          reason: "autofill engine did not load in this frame",
        }));
      }
...
// Runs autofill in every frame and merges results by plan index.
// Keep whichever frame actually found the element over "not in this frame".
async function fillFieldsInActiveTab(plans) {
  if (!Array.isArray(plans) || plans.length === 0) {
    return { results: [] };
  }
  ...
  if (!tab?.id) {
    return {
      error: "No active tab to fill.",
      hint: "Open the job application page and try again.",
    };
  }
  ...
  try {
    ...
    injection = await runAutofillEngine(tab.id, plans);
  } catch (err) {
    return {
      error: "Fill failed.",
      hint: err?.message || String(err),
  ...
  for (const result of all) {
    ...
    const existingMissing =
      existing.reason === "not in this frame" ||
      existing.reason === "autofill engine did not load in this frame";

    const resultMissing =
      result.reason === "not in this frame" ||
      result.reason === "autofill engine did not load in this frame";

    if (existingMissing && !resultMissing) {
      byIndex.set(result.index, result);
    }
  }
...
// Async message handler. Return true to keep the response channel open.
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg?.type === "SCAN_ACTIVE_TAB") {
    scanActiveTab()
      .then(sendResponse)
