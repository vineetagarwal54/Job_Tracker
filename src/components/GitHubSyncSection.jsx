...
export function GitHubSyncSection({ workspaces = [], workspaceJobCounts = {} }) {
  const [status, setStatus] = useState(null);
  // null = saved config predates workspace selection → every workspace syncs.
  const [syncWsIds, setSyncWsIds] = useState(null);
  ...
  // Only existing workspaces count, so a deleted workspace's id can't linger.
  const selectedWsIds = (syncWsIds ?? workspaces.map(w => w.id)).filter(id => workspaces.some(w => w.id === id));
  const toggleWorkspace = (id, on) =>
    setSyncWsIds(on ? [...selectedWsIds, id] : selectedWsIds.filter(x => x !== id));

  const save = () => run(async () => {
    const res = await api.saveConfig({ owner, repo, branch, enabled, token, workspaceIds: selectedWsIds });
    if (res.ok) setToken("");
    return res;
  }, "Settings saved.");
    ...
    <div style={boxStyle}>
      ...
      {status && status.lastSuccessAt && (
        <div style={{ fontSize: "12px", color: "#5a6070", marginBottom: "4px" }}>
          ...
          {status.syncedCount != null && ` · ${status.syncedCount} application${status.syncedCount === 1 ? "" : "s"} in applications.json`}
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginTop: "12px" }}>
        ...
        <FormField label="WORKSPACES TO SYNC">
          <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 16px", padding: "4px 0" }}>
            {workspaces.map(w => (
              <label key={w.id} style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px", color: "#e2e8f0", cursor: "pointer" }}>
                <input type="checkbox" checked={selectedWsIds.includes(w.id)} onChange={e => toggleWorkspace(w.id, e.target.checked)} />
                {w.name}
                <span style={{ color: "#5a6070" }}>({workspaceJobCounts[w.id] ?? 0})</span>
              </label>
            ))}
          </div>
        </FormField>
