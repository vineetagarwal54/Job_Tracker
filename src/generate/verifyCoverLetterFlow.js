...
const bundleFor = (baseId, job, jobAnalysis) => buildCoverLetterEvidence({ bank, job, analysis: jobAnalysis, selection: selectionFor(baseId) });
...
for (const item of cases) {
  const job = jobFor(item.title, item.jd); const jobAnalysis = analysis(item.variant, item.must);
  const bundle = bundleFor(item.id, job, jobAnalysis);
...
const normalBundle = buildCoverLetterEvidence({ bank, job: normalJob, analysis: normalAnalysis, selection: normalSelection });
...
const normal = await generateCoverLetter({
  ...
  apiKey: "fake", bank, job: normalJob, analysis: normalAnalysis, selection: normalSelection, generateDir: here,
});
...
async function invalidModelFallsBack(mutator) {
  ...
  let requestCount = 0;
  const result = await generateCoverLetter({ client: { request: async () => { requestCount += 1; return { text: JSON.stringify(unsafe), stopReason: "end_turn", usage: { input_tokens: 10, output_tokens: 20 } }; } }, apiKey: "fake", bank, job: normalJob, analysis: normalAnalysis, selection: normalSelection, generateDir: here });
...
let failureCalls = 0;
const failedModel = await generateCoverLetter({ client: { request: async () => { failureCalls += 1; throw new Error("simulated model outage"); } }, apiKey: "fake", bank, job: normalJob, analysis: normalAnalysis, selection: normalSelection, generateDir: here });
...
const independent = await generateCoverLetter({ client: { request: async () => { throw new Error("offline"); } }, apiKey: "fake", bank, job: jobFor("Software Engineer", "Build Python APIs and mobile products."), analysis: analysis("fullstack", ["python"]), selection: null, resumeText: localResume, generateDir: here });
