...
export function validateCoverLetter(value, bank, options = {}) {
  ...
  if (value.version !== 1 || typeof value.opening !== "string" || typeof value.closing !== "string" || !Array.isArray(value.bodyParagraphs) || value.bodyParagraphs.length !== 2 || !value.bodyParagraphs.every((item) => typeof item === "string") || !Array.isArray(value.claimEvidence)) {
  ...
  const claimEvidence = value.claimEvidence.map((claim) => ({ sentence: String(claim?.sentence || "").trim(), evidenceIds: [...new Set((claim?.evidenceIds || []).map(String).filter(Boolean))] }));
  if (!claimEvidence.length || claimEvidence.some((claim) => !claim.sentence || !claim.evidenceIds.length)) throw Object.assign(new Error("Every cover-letter factual claim must cite verified evidence IDs."), { code: "VALIDATION_FAILED" });
  return { version: 1, opening: paragraphs[0], bodyParagraphs: paragraphs.slice(1, 3), closing: paragraphs[3], claimEvidence, wordCount: words };
}
